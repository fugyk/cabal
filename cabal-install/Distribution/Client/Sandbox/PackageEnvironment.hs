...
initialPackageEnvironment sandboxDir compiler platform conf = do
  ...
  let initialConfig = commonPackageEnvironmentConfig sandboxDir
      installDirs   = if (Register.pkgenvSupported compiler conf)
...
setPackageDB sandboxDir compiler platform configFlags conf =
  if pkgenvSupported
    then configFlags {
      ...
      --configPkgenv = Flag $ sandboxDirHash sandboxDir
    }
    else configFlags {
  ...
  where pkgenvSupported = Register.pkgenvSupported compiler conf
...
createPackageEnvironmentFile verbosity sandboxDir pkgEnvFile compiler platform
  ...
  initialPkgEnv <- initialPackageEnvironment sandboxDir compiler platform conf
